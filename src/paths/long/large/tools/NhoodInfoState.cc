...
{    
     ...
     {    out << "Available commands:\n";
          out << "- H: help (see also the file doc/NhoodInfo)" << endl;
          ...
          if ( !SERVER )
               out << "- READ=id: show where read is placed on the assembly" << endl;
     ...
     {    out << "Core command type: a blank-separated list of arg=value "
          ...
          out << "- PURPLE: for multi-sample assemblies, if you supply a string "
               ...
               << "  flag edges present in only the first sample" << endl;
          ...
          out << "- ASPECT: default -1, if set provides aspect ratio" << endl;
     ...
     {    int64_t rid = line.After( "READ=" ).Int( );
          ...
          if ( rid < 0 || rid >= N )
          {    out << "That read id doesn't make sense." << endl;
               return False;    }
          ReadPathVec p;
          p.ReadOne( paths, rid );
          if ( p[0].size( ) == 0 ) out << "unplaced" << endl;
          else
          {    int offset = p[0].getOffset( );
               out << "placed fw starting at position " << offset
                    << " on edge " << p[0][0] << " (len=" << hb.Bases(p[0][0]) 
                    << ")" << endl;
               out << "full path = " << printSeq( p[0] ) << endl;    }
          return False;    }
     ...
     {    String s = x[i];
          ...
          {    if ( !engine.HasCounts( ) )
               {    out << "PURPLE only works if there is a file a.counts" << endl;
                    return False;    }
               ...
               if ( !OK )
               {    out << "Your PURPLE value doesn't make sense." << endl;
                    return False;    }
               PURPLE = y;    }
