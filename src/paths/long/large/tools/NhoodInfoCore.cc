...
void NhoodInfoCore( int argc, char *argv[] ) 
{
	...
	"optionally also as a PDF or PNG or SVG file.";
     ...
     free(cwd);
     if (INTERACTIVE) cout << "(loading)" << endl;
     else cout << Date( ) << ": loading" << endl;
...
void NhoodInfoEngine::SetState(const String& SEEDS,
    ...
    state.SVG = SVG;
     ...
     if ( PURPLE != "" )
     {    if ( !HasCounts( ) )
          {    cout << "PURPLE only works if there is a file a.counts" << endl;
               Scram(1);    }
          String y = PURPLE;
          Bool OK = True;
          ...
          if ( !OK )
          {    cout << "Your PURPLE value doesn't make sense." << endl;
               Scram(1);    }    }
