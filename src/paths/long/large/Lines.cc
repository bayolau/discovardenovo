...
{    double clock = WallClockTime( );
     ...
     {    int max_cell_verts = verts_mul * max_cell_paths;
          ...
          // cout << Date( ) << ": symmetrizing cells" << endl;
          int nb = bounds0.size( );     
          for ( int i = 0; i < nb; i++ )
     ...
     {    int e = ids[ie];
          ...
          {    int w = to_left[ line.front( )[0][0] ];
               ...
               line.push_front( {{eb}} );
               marked[eb] = True;
               for ( int i = 0; i < xpaths[bid].isize( ); i++ )
               for ( int j = 0; j < xpaths[bid][i].isize( ); j++ )
...
{    vec< pair<int,int> > line_ends( lines.size( ) );
     ...
     {    int e1 = line_ends[i].first, e2 = line_ends[i].second;
          if ( !BinMember( line_ends, make_pair( inv[e2], inv[e1] ) ) )
          {    
               #pragma omp critical
               {    cout << "Failed to find rc of line "
                         << e1 << ".." << e2 << "." << endl;
