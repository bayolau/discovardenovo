        ...
        ReadPathVec& paths, const Bool preclose_verbose )
{
     ...
     cout << TimeSince(iclock) << " used defining edge pairs" << endl;
     ...
     {    vec< vec<int> > pun1( pairs.size( ) ), pun2( pairs.size( ) );
          ...
          {    pids[j].append( px1[ edge_pairs[j].first ] );
               ...
               UniqueSort( pids[j] );    }    }
     cout << TimeSince(xclock) << " used selecting read pairs" << endl;
     ...
     cout << TimeSince(pclock) << " used in local assembly loop" << endl;
     ...
     {    double aclock = WallClockTime( );
          ...
          {    int edges = 0;
               ...
               {    for ( int j = 0; j < edge_pairs.isize( ); j++ )
                         all.Append( extra[j] );    }    }
          cout << TimeSince(aclock) << " used building all" << endl;
     ...
     cout << TimeSince(rclock) << " used reconstructing paths" << endl;
     ...
     cout << Date( ) << ": done with preclose" << endl;
