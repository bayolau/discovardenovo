...
void FinalPrint( ostringstream* pout )
{
     if ( pout != NULL )
     {
          #pragma omp critical
...
template<int L> void ImprovePath( const vec<int>& rstarts,
     const vec< pair<int64_t, pair<int,int> > >& locsx,
     ...
     path_improver::path_status& status )
{
     ...
     ostringstream* pout = NULL;
     if ( pimp.Logging( ) ) pout = new ostringstream;
     ...
     {    if ( p.size( ) > 0 )
          {    status = path_improver::old_better;
               ...
               {    *pout << "\nid = " << id << ", old better" << endl;
                    *pout << "was placed, but can't find seed" << endl;    }    }
          else
	  ...
	  {    if ( p.size( ) > 0 )
          ...
          {    if ( p.size( ) > 0 )
               {    status = path_improver::old_better;
                    if (pimp.show_old_better)
                    {    *pout << "\nid = " << id << ", old better" << endl;
                         *pout << "was placed, but found dead end" << endl;    }    }
               else
               {    status = path_improver::same;
                    if (pimp.show_same) 
                    {    *pout << "\nid = " << id << ", same" << endl;
     ...
     {    if ( p.size( ) > 0 )
          {    status = path_improver::old_better;
               if (pimp.show_old_better)
               {    *pout << "\nid = " << id << ", old better" << endl;
                    *pout << "was placed, but found no extensions" << endl;    }    }
          else
          {    status = path_improver::same;
               if (pimp.show_same) 
               {    *pout << "\nid = " << id << ", same" << endl;
     ...
     {    status = path_improver::same;
          if (pimp.show_same) 
          {    *pout << "\nid = " << id << ", same" << endl;
               *pout << "best path agrees with old path" << endl;    }
          FinalPrint(pout);
          return;    }
     ...
     {    status = path_improver::same;
          if (pimp.show_same) 
          {    *pout << "\nid = " << id << ", same" << endl;
               *pout << "wasn't placed, but new placement looks bad" << endl;    }
          FinalPrint(pout);
          return;    }
     ...
     {    status = path_improver::new_better;
          ...
          {    *pout << "\nid = " << id << ", new better" << endl;
               *pout << "old: empty" << endl;
               *pout << "new: " << starts[0] << ": " << printSeq(core) << endl;    }
          FinalPrint(pout);
          return;    }
     ...
     {    status = path_improver::indet;
          if (pimp.show_indet) 
          {    *pout << "\nid = " << id << ", indeterminate" << endl;
               print = True;    }    }
     if (print)
     {    *pout << "old: " << p.getOffset( ) << ":" << printSeq(p) << endl;
          vec<int> e;
          for ( int j = 0; j < (int) p.size( ); j++ )
               e.push_back( p[j] );
          basevector E = hb.Cat(e);
     ...
     const Bool IMPROVE_PATHS_LARGE, const Bool BETSYBOB )
{
     ...
     cout << Date( ) << ": improving paths" << endl;
     vec<int> to_left, to_right;
     hb.ToLeft(to_left), hb.ToRight(to_right);

     // First try large K.  Output is seeds.

     vec< pair<int64_t, pair<int,int> > > locsx;
     const int K = hb.K( );
     if (IMPROVE_PATHS_LARGE)
     {
          ...
          ForceAssertEq( L, K );
          vec< triple<uint64_t,int,int> > kmers_plus;
          cout << Date( ) << ": forming edges" << endl;
          vecbasevector edges( hb.E( ) );
          for ( int e = 0; e < hb.E( ); e++ )
          {    edges[e] = hb.EdgeObject(e);
               int ne = edges[e].size( );
               if ( ne > 0 ) edges[e].resize( ne + L - K );    }
          cout << Date( ) << ": hashing" << endl;
          MakeKmerLookup0H<L>( edges, kmers_plus );

          // Find unmapped reads.

          cout << Date( ) << ": finding unmapped reads" << endl;
          vec<int64_t> un;
          ...
          cout << Date( ) << ": traversing reads" << endl;
          #pragma omp parallel for
