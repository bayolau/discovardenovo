     ...
     const Bool GAP_CLEANUP )
{
     ...
     BinaryReader::readFile( work_dir + "/a." + FIN + "/a.lines", &lines );
     vec<int> llens, npairs;
     GetLineLengths( hb, lines, llens );
     BinaryReader::readFile( work_dir + "/a." + FIN + "/a.lines.npairs", &npairs );
     ...
     // cout << Date( ) << ": defining edge groups" << endl;
     ...
     // cout << Date( ) << ": computing nears" << endl;
     ...
     // cout << Date( ) << ": sorting links" << endl;
     ParallelSortSync( links, counts );

     // Compute edge groups.

     // cout << Date( ) << ": computing edge groups" << endl;
     vec<vec<int>> gp(nobj);
     for ( int e = 0; e < nobj; e++ )
     ...
     // cout << Date( ) << ": finalizing links" << endl;
     vec< pair<int,int> > accepted;
     ...
     {    int e1 = links[i].first, e2 = links[i].second;
          ...
          {    cout << "\n" << events << ". [" << counts[i] << "] " << e1 
                    << " --> " << e2 << endl;
               cout << "lines: " << tol[e1] << "[len=" << llens[tol[e1]] << "], " 
                    << tol[e2] << "[len=" << llens[tol[e2]] << "]" << endl;
               cout << "lefts: ";
               for ( int l = 0; l < nears1[e1].isize( ); l++ )
               {    int m = nears1[e1].NextDiff(l);
                    if ( l > 0 ) cout << ",";
               ...
               {    int m = nears2[e2].NextDiff(l);
                    ...
                    if ( m - l > 1 ) cout << "^" << m - l;
                    l = m - 1;    }
               cout << "\nleft edge group: " << printSeq( gp[e1] ) << endl;
               cout << "right edge group: " << printSeq( gp[e2] ) << endl;    }    }
     ...
     {    int low1 = LowerBound( a1, accepted[i].first );
          ...
          {    if (verbose)
               {    cout << "deleting " << accepted[i].first << " --> "
                         << accepted[i].second << "\n";    }
               adel[i] = True;    }    }
     ...
     vec< pair<int,int> > accepted2;
     vec<Bool> acdel( accepted.size( ), False );
     for ( int i = 0; i < na; i++ )
     {    int e1 = accepted[i].first, e2 = accepted[i].second;
          int re1 = inv[e1], re2 = inv[e2];
          if ( !BinMember( accepted, make_pair( re2, re1 ) ) )
          {    if ( !xa1[re2] && !xa2[re1] ) accepted2.push( re2, re1 );
               else acdel[i] = True;    }    }
     EraseIf( accepted, acdel );
     accepted.append(accepted2);
     UniqueSort(accepted);
     cout << Date( ) << ": deleting " << Sum(acdel) << " gaps and adding " 
          << accepted.isize( ) - na << " gaps to force symmetry" << endl;
     ...
     {    int e1 = accepted[i].first, e2 = accepted[i].second;
          int re1 = inv[e1], re2 = inv[e2];
          int ri = BinPosition( accepted, make_pair( re2, re1 ) );
          ForceAssertGe( ri, 0 );
          inv[ nold + i ] = nold + ri;    }
     ...
     // PRINT(events);
     if (verbose) cout << "\n";
     cout << Date( ) << ": done making gaps, time used = "
