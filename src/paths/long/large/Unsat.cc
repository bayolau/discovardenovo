...
void MergeClusters( const vec< vec< pair<int,int> > >& x,
     vec< vec< pair<int,int> > >& y, const vec< vec<int> >& n, const int N )
...
void PrintClusters( const vec< vec< pair<int,int> > >& xs, 
     map< pair<int,int>, int >& mult, const String& txt )
{    Ofstream( out, txt );
     for ( int i = 0; i < xs.isize( ); i++ )
     {    vec< pair<int,int> > d = xs[i];
          out << "\n[" << i << "]\n";
          Sort(d);
          vec<int> all;
          ...
          {    int k = d.NextDiff(j);
               ...
               all.push_back( e1, e2 );
               out << e1 << "," << e2 << " [" << mult[ d[j] ] << "]" << endl;
               j = k - 1;    }
          UniqueSort(all);
          out << printSeq(all) << endl;    }    }

void Unsat( const HyperBasevector& hb, const vec<int>& inv, 
     const ReadPathVec& paths, vec< vec< pair<int,int> > >& xs, 
     const String& work_dir, const int A2V )
{
     ...
     cout << Date( ) << ": finding unsatisfieds" << endl;
     vec<vec< pair<int,int64_t> >> unsats( hb.EdgeObjectCount( ) );
     ...
     // Create link multiplicity map.

     cout << Date( ) << ": creating multiplicity map" << endl;
     ...
     {    UniqueSort( unsats[e] );
          ...
          {    int j;
               ...
               mult[ make_pair( e, unsats[e][i].first ) ] = j - i;
               i = j - 1;    }    }

     // Delete duplicate links.

     cout << Date( ) << ": economizing links" << endl;
     #pragma omp parallel for
     for ( int e = 0; e < unsats.isize( ); e++ )
     ...
     cout << Date( ) << ": forming neighborhoods" << endl;
     ...
     cout << Date( ) << ": forming initial clusters" << endl;
     xs.clear( );
     #pragma omp parallel for
     ...
     {    for ( int m = 0; m < unsats[id1].isize( ); m++ )
          {    int id2 = unsats[id1][m].first;
               ...
               #pragma omp critical
               {    xs.push_back(x);    }    }     }
     cout << Date( ) << ": start sort" << endl; // XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
     double clock = WallClockTime( ); // XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
     sortInPlaceParallel(xs.begin(),xs.end());
     cout << TimeSince(clock) << " used sorting" << endl; // XXXXXXXXXXXXXXXXXXXXXXX
     Unique(xs);

     // Merge clusters.

     cout << Date( ) << ": merging clusters" << endl;
     ...
     cout << TimeSince(mclock) << " used merging" << endl;
     ...
     {
     cout << Date( ) << ": start overlap-based merging" << endl;
     ...
     int nxs = xs.size( );
     for ( int i = 0; i < nxs; i++ )
     {    vec< pair<int,int> > d = xs[i], rd;
          for ( int j = 0; j < d.isize( ); j++ )
               rd.push( make_pair( inv[ d[j].second ], inv[ d[j].first ] ) );
          xs.push_back(rd);    }
     MergeClusters( xs, xs, n, hb.EdgeObjectCount( ) );
