...
void FinalFiles( const HyperBasevector& hb, const vec<int>& inv, const 
     ...
     const String& X, map<String,GapToyResults>& res, const String& SAMPLE, 
     ...
     const Bool SAVE_FASTA )
{
     ...
     {    vec< vec< pair<int,int> > > hitsx_alt;
     ...
     // cout << "finding scaffold lines, mem usage = "
     ...
     {    vec<int> lens;
          ...
          {    ostringstream out2;
               ...
               {    cout << "Something changed!  We expected " << res[X].nedges
     ...
     {    vec<vec<vec<vec<int>>>> linesy;
          ...
          {    sout << "# assembly statistics" << endl;
               sout << "# please see also frags.dist.png in parent directory"
                    << endl;
               sout << "contig line N50: " << ToStringAddCommas(contigN50) << endl;
               ...
               {    sout << "There are " << ToStringAddCommas(nreads) 
                         << " reads of mean length " << setiosflags(ios::fixed)
                         << setprecision(1) << stats.mean_read
                         << resetiosflags(ios::fixed) << " and mean base quality "
                         << setiosflags(ios::fixed) << setprecision(1) 
                         << stats.mean_qual << "." << endl;
               ...
               sout << "Estimated chimera rate in read pairs "
                    ...
                    << PERCENT_RATIO( 2, chim, chim_total ) << "." << endl;
               ...
               {    sout << "genomic read coverage, using 1 kb+ scaffolds for "
                         << "genome size estimate: " << setiosflags(ios::fixed)
                         << setprecision(1) 
               ...
               {    Bool concatenate = ( SAMPLE == "NA12878" || SAMPLE == "HCC1954"
                    ...
                    sout << "N50 perfect stretch = " << n50 << endl; 
                    PerfStatLogger::log(
          ...
          {    getline( in, line );
               if ( in.fail( ) ) break;
               if ( !line.Contains( "#", 0 ) ) cout << line << endl;    }
     ...
     if ( ALIGN_TO_GENOME && IsRegularFile( work_dir + "/genome.fastb" ) )
     {    // cout << Date( ) << ": assaying misassemblies" << endl;
          AssayMisassemblies( hb, inv, hitsx, linesx, final_dir );    }

     // Compute edge coverage.

     // cout << Date( ) << ": computing edge coverage" << endl;
