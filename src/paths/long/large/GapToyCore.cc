...
int GapToyCore( int argc, char *argv[] )
{
     ...
     {    if ( !IsRegularFile( REFHEAD + ".fasta" ) )
	  ...
	  vecString onames; // original read names
		  ...
		  names_out << onames[i].SafeBefore(" ").SafeBefore("|") << endl;
	  }
     }
     ...
     {    Ofstream( rout, work_dir + "/README" );
          ...
          if (SAVE_60) rout << "a.60      - initial graph from 60-mers" << endl;
          rout << "a.200     - initial graph from 200-mers" << endl;
          if (SAVE_PATCHED) rout << "a.patched - patched graph" << endl;
     ...
     {    
          ...
          {    ExtractReads( SAMPLE, species, READS, SELECT_FRAC, READS_TO_USE, 
               ...
               {    cout << "\nUh-oh, internal error, sample name and size files "
                         << "have different record counts." << endl
                         << "Giving up." << endl;
                         Scram(1);    }    }
          else
     ...
     {    double lclock = WallClockTime( );
          ...
          {    Mkdir777( work_dir + "/a.patched" );
               ...
               cout << Date( ) << ": a.patched written" << endl;
     ...
     {    double pclock = WallClockTime( );
          bases.destroy( );
          cout << Date( ) << ": writing a.fin files" << endl;
     ...
     cout << "\nrun started " << start_time << ", completed " << Date( ) << endl;
