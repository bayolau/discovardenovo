...
void ExtendReadPlaces( vec<read_place>& places_part, pqsum& candidates,
     ...
     for ( int i = 0; i < places_part.isize( ); i++ ) {
       ...
       int e = p.E(0), s = p.P( );
       for ( int j = 0; j < K; j++ ) {
	 if (log) cout << Base::val2Char(hb.EdgeObject(e)[s+j]) << "  " << Base::val2Char(b[j]) << " " << ToString(int(q[j])) ;
	 ...
	 min_ext = Min( min_ext, hb.EdgeLengthKmers(e) );
       ...
       for ( int j = 0; j < hb.From(v).isize( ); j++ ) {
	 if (log) cout << "Edge " << j  << endl;
	 int e = hb.EdgeObjectIndexByIndexFrom( v, j );
	 for ( int l = 0; l < min_ext; l++ ) {
	   if (log) cout << Base::val2Char(hb.EdgeObject(e)[K-1+l]) << "  " << Base::val2Char(b[bpos+l]) << " " << ToString(int(q[bpos+l])) ;
	 ...
	 return; 
       ...
       if (log) cout << "Edge Chosen " << ids[0]  << endl;
		  ...
		  bool log )
{
     ...
     //     cout << "Finding places for " << bases.size() << " reads" << endl;

     // Build data structures.

     // cout << Date( ) << ": building data structures" << endl;
     ...
     */
     ...
     {    int K = 60;
	  ...
	  //	  cout << Date( ) << ": creating partials for K=" << K << endl;
          ...
          {    if ( partials[id].nonempty( ) )
               {    
		 if ( !found[id]) 
		   //		   cout << "placing read " << id << " using K = " << K << endl;
		 found[id] = True;    }    }    }

     //     cout << "Places: " << partials[0].size() << endl;
     //     cout << Date( ) << ": aligning seeding on kmers" << endl;
     ...
     {    const basevector& b = bases[id];
	  ...
	  {   PLACES[id].push_back( candidates.top( ) );
	    ...
	    #pragma omp critical
	    {
	      out << "\nplacements of read: " << id << endl;
	      for ( int j = 0; j < (int) PLACES[id].size( ); j++ )
		out << "[" << j+1 << "] " << PLACES[id][j] << endl;    }    
          */
