 ...
 */
ostream& operator<<(std::ostream& os, const _BaseVec& v )
{
 ...
 */
ostream& printq(std::ostream& os, const _QualVec& v )
{
  for (size_t i = 0; i < v.size(); ++i ) {
    if ( v[i] < 0 )
      os << "-";
...
private:
  ...
  vector<double> _Q2q;
  vector<double> _Q2p;
  vector<double> _Q2logq;
  vector<double> _Q2logp;


public:

  static double Q2p(int Q) { ForceAssertGe(Q, 0); return me._Q2p[Q]; };
  static double Q2q(int Q) { ForceAssertGe(Q, 0); return me._Q2q[Q]; };
  static double Q2logp(int Q) { ForceAssertGe(Q, 0); return me._Q2logp[Q]; };
  static double Q2logq(int Q) { ForceAssertGe(Q, 0); return me._Q2logq[Q]; };
 ...
 *
 */
void init_prior( const _BaseVec& read, const _QualVec& quals, vector<BaseProb>& prior, bool debug = false ) {
  size_t read_len = read.size();
  prior.resize(read_len, BaseProb(4,0.));

  if ( debug )
    for ( size_t i = 0; i < prior.size(); ++i )
      cout << "read,qual[" << i << "]=" << Base::val2Char(read[i]) << "," << quals[i] << endl;
  ...
  if ( debug ) {
    for ( size_t i = 0; i < prior.size(); ++i )
      cout << "base,qual=" << static_cast<int>(read[i]) << "," << quals[i] << "=== prior[" << i << "]=" << prior[i][0] << "," << prior[i][1] << "," << prior[i][2] << "," << prior[i][3] << endl;
    cout  << endl;
  }
}
 ...
 */
double estimate_friends( const _BaseVecVec& reads, const _QualVecVec& quals, const _BaseVec& truth, double a, vector<double>& pfriend, bool debug = false )
{
  double rmsdiff = 0.0;
...
#pragma omp critical 
	...
	cout << "P1=" << p1 << endl;
	cout << "P2=" << p2 << endl;
	cout << "READ0:" << endl << reads[0] << endl;;
	cout << "TRUTH:" << endl << truth << endl;
	cout << "READ0_Q:" << endl;
	printq(cout, quals[j]) << endl;
	double not_friend = (1.0-a)*p2 / ( a*p1 + (1.0-a)*p2 );
	cout << "friend=" << new_friend << ", not_friend=" << not_friend << ", sum=" << new_friend+not_friend << endl;
	}
    }
    ...
    pfriend[j] = min(max(new_friend, 0.0001),1.0-0.0001);
 ...
 *
 */
void estimate_A( vector<double>& pfriend, double& a_return )
{
  long double a = 0.0;
 ...
 */
void estimate_truth( const _BaseVecVec& reads, const _QualVecVec& quals, const vector<double>& pfriend, const vector<BaseProb>& t_prior, _BaseVec& truth, _QualVec& truthq, bool final, bool debug = false, const size_t serialno = 0 )
{
  ...
  vector<int> quality_sum( truth.size(), 0);
...
#pragma omp critical
	  cout << "pfriend[j]=" << pfriend[j] << ", logqij=" << logqij << ", logpij=" << logpij << ", mis=" << mis << ", hit=" << hit << endl;
	}
    ...
    if ( count < 3 ) { 		// NIW TEMPORARY - and quality_sum < X ?
      ...
      if ( debug && ( i == 24 || i == 25 ) )
	  cout <<  "TOOK BASE FROM READ due to lack of evidence" << endl;
      ...
      if ( debug && ( i == 24 || i == 25 ) ) { 	// NIW TEMPORARY
	cout << "p[]=" << exp(p[0]) << "," << exp(p[1]) << "," << exp(p[2]) << "," << exp(p[3]) << endl;
	cout << "prior=" << t_prior[i][0] << "," << t_prior[i][1] << "," << t_prior[i][2] << "," << t_prior[i][3] << endl;
      }
	...
	p[idx] =  p[idx] + gamma*log(t_prior[i][idx]) ;
      ...
      if ( debug && ( i == 24 || i == 25 ) ) { // NIW TEMPORARY
      	cout << "with prior p[]=" << p[0] << "," << (p[1]) << "," << (p[2]) << "," << (p[3]) << endl;
      	cout << "prior=" << t_prior[i][0] << "," << t_prior[i][1] << "," << t_prior[i][2] << "," << t_prior[i][3] << endl;
            }
...
#pragma omp critical 
	...
	cout << "serialno=" << serialno << endl; 
      	cout << "withOUT prior p[])=" << exp(p_tmp[0]) << "," << (exp(p_tmp[1])) << "," << (exp(p_tmp[2])) << "," << (exp(p_tmp[3])) << endl;
      	cout << "with prior p[]=" << p[0] << "," << (p[1]) << "," << (p[2]) << "," << (p[3]) << endl;
      	cout << "with prior before exp p[]=" << p_tmp2[0] << "," << (p_tmp2[1]) << "," << (p_tmp2[2]) << "," << (p_tmp2[3]) << endl;
	log_domain_norm( p_tmp2 );
      	cout << "with prior after lognorm-exp p[]=" << exp(p_tmp2[0]) << "," << exp(p_tmp2[1]) << "," << exp(p_tmp2[2]) << "," << exp(p_tmp2[3]) << endl;
      	cout << "prior=" << t_prior[i][0] << "," << t_prior[i][1] << "," << t_prior[i][2] << "," << t_prior[i][3] << endl;
	cout << "gamma=" << gamma << endl;

	for ( size_t ji = 0; ji < pfriend.size(); ji++ )
	    cout << "pfriend[" << ji << "]=" << pfriend[ji] << endl;
	}
      }
#endif
	...
	p[idx] /= sum;
	p[idx] = max<long double>(1e-5, min<long double>(1.0-1e-5, p[idx]));
      }
...
//#pragma omp critical
//      cout << "imax=" << imax << ", index[0]=" << index[0] << ", p[0]=" << p[0] << ", p[1]=" << p[1] << ", p[2]=" << p[2] << ", p[3]=" << p[3] << endl;
      ...
      //    cout << "p[imax]=" << p[imax] << ", truthq[" << i << "]=" << truthq[i] << endl;

      if ( debug && ( i == 24 || i == 25 ) )
	cout << "WE DECIDED=" << Base::val2Char(truth[i]) << endl;
    }
  }
}
...
void hack_q2( const _BaseVecVec& call, const _QualVecVec& callq, const vector<double>& pfriend, const _BaseVec& t, _QualVec& q)
{
	...
	vector<long double> credit(4,0.);
	vector<long double> credit2(4,0.);
	if ( t[i] != call[0][i] ) {
	    ...
	    double perror = max<long double>( 1e-4, min<long double>( 0.75 , (1.0 - credit[t[i]] ) ) );

	    if ( perror > 0.74 ) {
		cout << "high error: credit=" << credit[0] << "," << credit[1] << "," << credit[2] << "," << credit[3] << endl;
		cout << "___________ credit=" << credit2[0] << "," << credit2[1] << "," << credit2[2] << "," << credit2[3] << endl;
		double alterror = max<long double>( 1e-4, min<long double>( 0.75 , (1.0 - credit2[t[i]] ) ) );
		cout << "alterror: " <<  alterror << endl;
	    }
 ...
 *
 */
void run_EMEC3( const _BaseVecVec& call, const _QualVecVec& callq, _BaseVec& t, _QualVec& q, vector<double>& pfriend, int& trim_to, const unsigned debug, const size_t serialno )
{
  ...
  if ( 0 && serialno == 3152 ) {	// NIW TEMPORARY
    cout << "orig0_pos=" << 66 << ": " << Base::val2Char(call[0][66]) << "(" << callq[0][66] << ")" << endl;
    cout << "prior=" << t_prior[66] << endl;
    cout << "truth=" << Base::val2Char(t[66]) << endl;
  }
  ...
  for ( ; iter < maxiter; ++iter ) {
	...
	if ( debug > 0 )
	    cout << "short circuting due to lack of friends" << endl;
	t = call[0];
	q = callq[0];
...
#if 0
    if ( 0 && serialno == 3152 )  {			// NIW TEMPORARY
      vector<long long> evidence(4,0);

      for ( size_t j = 1; j < call.size(); ++j ) {
	if ( call[j][66] == ' ' ) continue;
	cout << ( ( pfriend[j-1]>0.5) ? "FRIEND" : "friend" );
	cout << " " << j << "(" << pfriend[j-1] << ")" << " " << ( call[j][66] == 32 ? ' ' : Base::val2Char(call[j][66]) ) << "(" << callq[j][66] << ")" <<endl;
	if ( call[j][66] != 32 )
	  evidence[call[j][66]]+=callq[j][66];
      }

      cout << "evidence[A]=" << evidence[0] << ", [C]=" << evidence[1] << ", [G]=" << evidence[2] << ", [T]=" << evidence[3] << endl;
    }
#endif
...
#pragma omp critical
	  {
	  cout << Date() << ": EM converged to wrong mode: id=" << serialno << ", pfriend[0]=" << pfriend[0] << endl;
	  }
      t = call[0];
      q = callq[0];
...
#pragma omp critical
	  {
	  cout << Date() << ": EM didn't converge: id=" << serialno << ", pfriend[0]=" << pfriend[0] << ", olddiff=" << olddiff << endl;
	  }
#if 0
      t = call[0];
      q = callq[0];
