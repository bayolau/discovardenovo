     ...
     vec<int>& seeds )
{
     ...
     {    vec<String> seedsx;
          ...
          {    String s = seedsx[i];
               ...
               {    if ( !s.Contains( ".." ) )
                    {    cout << "Illegal seed notation." << endl;
                         Scram(1);    }
     ...
     {    if ( seeds[i] < 0 || seeds[i] >= hb.EdgeObjectCount( ) )
          {    cout << seeds[i] << " is an illegal seed value" << endl;
               Scram(1);    }    }    }
...
Bool ParseSeeds( const HyperBasevectorX& hb, const vec<int>& inv,
     ...
     const vec< pair<int,ho_interval> >& ambint, Bool& ambflag,
     const vec< vec< pair<int,int> > >& hits, const String& SEEDS, 
     const int RANDOM_SEED, const String& SEEDS_MINUS, vec<int>& seeds,
     const int max_seeds, ostream& tout )
{
     if ( RANDOM_SEED >= 0 ) srandomx(RANDOM_SEED);
     seeds.clear( );
     vec<int> seeds_minus;
     ...
     {    vec<String> seedsx;
          ...
          {    String s = seedsx[i];
               ...
               {    basevector b(s);
                    const int L = 20;
                    tout << "query has length " << b.size( ) << endl;
                    if ( b.isize( ) < L )
                    {    tout << "Sorry, need at least " << L << " bases." << endl;
                         return False;    }
                    basevector rb(b);
                    ...
                    seeds.append(all);
                    tout << "using seeds";
                    if ( all.size( ) <= 40 ) tout << " " << printSeq(all) << endl;
                    else
                    {    for ( int i = 0; i < 20; i++ )
                              tout << " " << all[i];
               ...
               {    int l = 0;
                    ...
                    if ( s.Contains( ":" ) )
                    {    if ( !s.After( ":" ).IsInt( ) )
                         {    tout << "Illegal seed notation " << s << "." << endl;
                              return False;    }
                         l = s.After( ":" ).Int( );
                         s = s.Before( ":" );    }
                    int n = s.Int( );
               ...
               {    if ( genome_names.size( ) == 0 )
			      ...
			      << endl;
                    ...
                    if ( g < 0 ) 
                    {    tout << "Unknown genome record name." << endl;
                         return False;    }
                    if ( !s.Between( ":", "-" ).IsInt( ) 
                         || !s.After( "-" ).IsInt( ) )
                    {    tout << "Illegal seed notation " << s << "." << endl;
                    ...
                    {    if ( g == ambint[j].first && Subset( 
                         ...
                         {    tout << "You've specified a reference locus "
                                   << "consisting "
                                   << "entirely of ambiguous bases." << endl;
                              ambflag = True;
                              break;    }    }
     ...
     {    if ( seeds[i] < 0 || seeds[i] >= hb.E( ) )
          {    tout << seeds[i] << " is an illegal seed value" << endl;
               return False;    }    }
     return True;    }
