...
ostream& operator<<( ostream &out, SmithWatScore &SWs ) {
  ...
  out << "first_a,last_a = " << first_a << " " << last_a << endl ; 
  out << "first_b,last_b = " << first_b << " " << last_b << endl ; 

  for( int i = first_a; i < last_a+2; i++ ) {
    //    out << " SmithWateScore.cc  SWs.firstB( i ) = " <<  SWs.firstB( i ) << " i = " << i << endl ; 
    ...
    for ( int j = SWs.firstB( i ) -1; j <= min( last_b, SWs.lastB( i ) )+1; j++ ) {
      //      out << " SmithWatScore.cc: i = " << i << " j = " << j << endl ;
  ...
  out << " a # is printed at beg_a, beg_b and at end_a, end_b " << endl ; 
  out << " a , is printed after each element for which both indices are multiples of 5 " << endl ; 
  out << " a @ is printed after each element for which both indices are multiples of 25 " << endl ; 
  out << " a . is printed after each element for which j is a multiple of 5 but i is not " << endl ; 
  out << " -999 means unitialized; 999 means set to MAX_COST " << endl ; 
  out << " SWs.beg_a_ = " << SWs.beg_a_  << " SWs.end_a_ = " << SWs.end_a_ 
       << " SWs.beg_b_ = " << SWs.beg_b_  << " SWs.end_b_ = " << SWs.end_b_  << endl ;
  return     out << endl ; 
}
