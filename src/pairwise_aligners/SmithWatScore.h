...
public:  
  SmithWatScore( int length, int bandwidth, int beg_a, int end_a, int length_a, 
    ...
    if ( bandwidth > maxbandwidth ) cout << " bandwidth = " << bandwidth << endl ; 
  ...
  int firstA( int b ) { 
    return std::max( {b + beg_a_ - beg_b_ - bandwidth_ ,
				    beg_a_ - bandwidth_, 0} ) ; }
  int lastA( int b ) { return std::min( {b + beg_a_ - beg_b_ + bandwidth_ ,
				   length_ + beg_a_ + bandwidth_ , length_a_ - 1} ) ; }
  int firstB( int a ) { 
    return std::max( {a + beg_b_ - beg_a_ - bandwidth_ + 1,
				    beg_b_ - bandwidth_, 0} ) ; }
  int lastB( int a ) { return std::min( {a + beg_b_ - beg_a_ + bandwidth_ ,
  ...
  inline Float& operator() ( int x, int y ) { 
    ...
    if( ( ( truex >= -1 ) && ( truey >= -1 ) && ( Abs( truex - truey ) <= bandwidth_ + 1 ) 
      ...
      cout << " SmithWatScore.h truex = " << truex << " truey = " << truey << " bandwidth = " << bandwidth_ << " x = " << x << " y = " << y << endl ; 
      Assert( false ) ; 
    } 
    Assert( truex >= -1 ); 
    Assert( truey >= -1 ); 
  ...
  int ArrayRefs() { return array_refs_ ; } ; 
